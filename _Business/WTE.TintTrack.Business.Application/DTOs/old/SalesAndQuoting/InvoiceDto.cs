...
public class InvoiceDto : GuidKeyedAuditableEntity, ICodedEntity
{
    ...
    public ICollection<WorkLogDto> WorkLogs { get; set; } = new HashSet<WorkLogDto>(); // For hourly/labor billing if T&M
    ...
    public bool CheckIfFullyPaid()
    {
        // Calculate the total balance to be paid
        var totalAmount = Subtotal + TaxAmount - DiscountAmount + Adjustments.Sum(a => a.AdjustmentAmount) - CreditMemos.Sum(c => c.CreditAmount) + LateFees.Sum(l => l.LateFeeAmount);
